...
class MyJaccardSimilarity(BaseEstimator, TransformerMixin):
    """my jaccard similarity on paired data."""
    ...
    def transform(self, X):
        ...
        for i, e1 in enumerate(X):
            x = e1[0]
            y = e1[1]
            Xt.append(jac.get_raw_score(ws_tok_set.tokenize(x), ws_tok_set.tokenize(y)))
        Xt = np.asarray(Xt)
        return Xt.reshape((n_samples, 1))


class MyCosineSimilarity(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X2 = []
        for e in X[:, 0]:
            X1.append([float(i.strip()) for i in e.replace("[", "").replace("]", "").split(",")])
        for e in X[:, 1]:
            X2.append([float(i.strip()) for i in e.replace("[", "").replace("]", "").split(",")])
