...
unzip_dir = os.path.join('E:/pubmed-medline/pubmed/data')  # path to unzip tar file
# unzip_dir = os.path.join('/ubuntu/home/pubmed/data')  # path to unzip tar file
...
def update():
    ...
    if file_list:
        ...
        if is_update:
            ...
            subprocess.call(
                ['wget', 'ftp://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/non_comm_use.A-B.xml.tar.gz', '--directory',
                 download_dir])
            if not os.path.isdir(unzip_dir): os.mkdir(unzip_dir)
            subprocess.call(
                ['tar', '-xzf', os.path.join(download_dir, 'non_comm_use.A-B.xml.tar.gz'), '--directory', unzip_dir])
        else:
...
def process_file(date_update, fraction=0.01):
    ...
    path_rdd = sc.parallelize(path_sample, numSlices=2000)  # use only example path
